    ...
    */
    ...
    private ObjectMap<Actor, Path> pathMap;
    ...
    private void initPosition(){
        ...
        pathMap.put(panel, new Path(0, stage.getHeight(), 0, stage.getHeight() - panel.getHeight()));
        pathMap.put(leftFrame, new Path(-leftFrame.getWidth(), (stage.getHeight()-leftFrame.getHeight())/2,
                stage.getWidth()*0.0625f, (stage.getHeight()-leftFrame.getHeight())/2));
        pathMap.put(rightFrame, new Path(stage.getWidth(), (stage.getHeight()-rightFrame.getHeight())/2,
                stage.getWidth()*0.53f, (stage.getHeight()-rightFrame.getHeight())/2));
        pathMap.put(bar, new Path((stage.getWidth()-bar.getWidth())*0.5f, -bar.getHeight(),
                (stage.getWidth()-bar.getWidth())/2, stage.getHeight()*0.1f));


        ObjectMap.Entries<Actor, Path> iterator = pathMap.iterator();
        while (iterator.hasNext){
            ObjectMap.Entry<Actor, Path> next = iterator.next();
            next.key.clearActions();
            if(isAppear)
                next.key.setPosition(next.value.x2, next.value.y2);
            else
                next.key.setPosition(next.value.x1, next.value.y1);
        }
    ...
    public void appear(float time){
        ...
        panel.addAction(Actions.moveTo(pathMap.get(panel).x2, pathMap.get(panel).y2, time, Interpolation.pow2));
        leftFrame.addAction(Actions.moveTo(pathMap.get(leftFrame).x2, pathMap.get(leftFrame).y2, time, Interpolation.circle));
        rightFrame.addAction(Actions.moveTo(pathMap.get(rightFrame).x2, pathMap.get(rightFrame).y2, time, Interpolation.circle));
        bar.addAction(Actions.moveTo(pathMap.get(bar).x2, pathMap.get(bar).y2, time, Interpolation.pow2));

        background.addAction(Actions.fadeIn(time));
        gameCircle.addAction(Actions.alpha(0));
        gameCircle.addAction(Actions.fadeIn(time));
        streamEnergy.addAction(Actions.sequence(Actions.delay(time*0.8f), Actions.run(() -> streamEnergy.start())));
        //
    }
    public void disappear(float time){
        ...
        panel.addAction(Actions.moveTo(pathMap.get(panel).x1, pathMap.get(panel).y1, time, Interpolation.pow2));
        leftFrame.addAction(Actions.moveTo(pathMap.get(leftFrame).x1, pathMap.get(leftFrame).y1, time, Interpolation.circle));
        rightFrame.addAction(Actions.moveTo(pathMap.get(rightFrame).x1, pathMap.get(rightFrame).y1, time, Interpolation.circle));
        bar.addAction(Actions.moveTo(pathMap.get(bar).x1, pathMap.get(bar).y1, time, Interpolation.pow2));

        background.addAction(Actions.fadeOut(time));
        gameCircle.addAction(Actions.fadeOut(time));
        streamEnergy.addAction(Actions.sequence(Actions.delay(time*0.8f), Actions.run(() -> streamEnergy.stop())));
